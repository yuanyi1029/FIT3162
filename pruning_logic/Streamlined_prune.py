...
def main_pruning_loop(model, block_level_dict, uniform_pruning_ratio, block_fine_tune_epochs, channel_fine_tune_epochs, device, type):
    pruned_model = model
    ...
    if type in ["BOTH", "BLOCK"]:
        ...
        if block_fine_tune_epochs > 0:
            pruned_model = main_finetune_model(pruned_model, block_fine_tune_epochs, device)

    # 2. Channel (Uniform) Pruning AFTER Block Pruning
    if type in ["BOTH", "UNIFORM"]:
        pruned_model = uniform_prune_and_depthwise_collapse(pruned_model, uniform_pruning_ratio)
        if channel_fine_tune_epochs > 0:
            pruned_model = main_finetune_model(pruned_model, channel_fine_tune_epochs, device)
